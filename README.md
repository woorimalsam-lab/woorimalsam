...
일정과 메모를 **어느 기기에서든 같은 구글 계정으로 동기화**하는 개인용 웹앱입니다.
공개 URL로 배포하면 휴대폰·학교 PC·집 PC 어디서나 접속할 수 있습니다.

## 주요 기능
- **일정 분류**: 등록하는 일정을 🟣 개인 / 🔵 업무 / 🟢 교과로 나눠 색깔로 구분합니다. 상단 칩(개인/업무/교과/학사)으로 원하는 종류만 골라 볼 수 있습니다.
- **학사일정 자동 표시**: 학교 학사일정 구글시트를 반영해 🟠 학사(시험·행사)와 🔴 휴일을 달력에 미리 채워 둡니다. (읽기 전용 — 수정은 시트에서)
- **일정·메모 동기화**: 로그인하면 등록한 일정과 메모가 **모든 기기에서 실시간 공유**됩니다(클라우드=Firebase Firestore 저장).
- **메모(구글 Keep 스타일)**: 컬러 카드 · 고정(핀) · 검색 · 항목(라벨)별 정리.
- **다크 모드**: 오른쪽 위 🌙 버튼으로 전환.

> 참고: 이전에는 일정을 구글 캘린더에 저장했지만, 로그인 때 뜨는 "확인되지 않은 앱" 경고를 없애기 위해 **캘린더 연동을 빼고 앱 자체 클라우드 저장**으로 바꿨습니다. 일정은 이제 이 앱 안에서만 관리됩니다(구글 캘린더 앱에는 표시되지 않음). 아래 설정의 **5단계(캘린더 API)는 더 이상 필요 없습니다.**
